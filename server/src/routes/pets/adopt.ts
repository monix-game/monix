...
export const adoptPet = new Elysia()
  ...
  .post('/adopt', async ({ authUser, set }) => {
    ...
    const result = await mutateUserAndSave<AdoptOutcome>(user_uuid, async fetchedUser => {
      ...
      if ((fetchedUser.money || 0) < 10000) {
        return {
          changed: false,
          value: { ok: 'error', status: 400, error: 'Insufficient funds to adopt a pet' },
        };
      }
      ...
      const pet: IPet = {
        ...
        exp: 0,
        is_dead: false,
        rarity: Math.random() < 0.08 ? 'rare' : 'common',
        bond: 0,
        passive_earned: 0,
