...
type HireOutcome =
  ...
  | { ok: 'success'; message: string; money: number; sailors: { levels: number[]; last_collected_at?: number } };
...
export const hireSailor = new Elysia()
  ...
  .post('/sailors/hire', async ({ authUser, set }) => {
    ...
    const result = await mutateUserAndSave<HireOutcome>(
      user_uuid,
      async fetchedUser => {
        fetchedUser.fishing ??= {
        ...
        if (levels.length >= SAILOR_MAX_LEVEL) {
          return { changed: false, value: { ok: 'error', status: 400, error: `You cannot hire more than ${SAILOR_MAX_LEVEL} sailors.` } };
        }
