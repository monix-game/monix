...
export async function getCurrentPrice(resourceId: string): Promise<number> {
  ...
  try {
    const resp = await api.get(`/market/price/${resourceId}`);
    if (resp && resp.success) {
      const payload: any = resp.data

      let price: number | undefined
      if (payload && payload.data && typeof payload.data.price === 'number') price = payload.data.price
...
export async function getPrices(): Promise<{ [resourceId: string]: number }> {
  try {
    ...
    if (resp && resp.success) {
      const payload: any = resp.data
      const prices: { [resourceId: string]: number } = {}
...
export async function getPriceHistory(resourceId: string, hoursBack: number = 2): Promise<Array<{ time: number; price: number }>> {
  const now = Date.now()
  ...
  try {
    const resp = await api.get(`/market/history/${resourceId}?hoursBack=${hoursBack}`)
    if (resp && resp.success) {
      const payload: any = resp.data
...
export async function getPriceStats(resourceId: string, hoursBack: number = 24): Promise<{
  min: number
  max: number
  avg: number
  current: number | null
